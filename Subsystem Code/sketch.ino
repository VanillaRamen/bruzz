...
void loop() {
  ...
  while (digitalRead(13) == LOW) { // switch is set to LED
    ...
    // analogWrite(6, RED); // RGB LED color write loop
    // analogWrite(5, GREEN);
    // analogWrite(3, BLUE);
    // delay(10);
    // RED++; // cycle through colors
    // GREEN += 2; // (currently only a rough color loop)
    // BLUE += 3;
    // if (RED == 256) { RED = 64; }
    // if (GREEN == 256) { GREEN = 64; }
    // if (BLUE == 256) { BLUE = 64; }
    // delay(20);
  } // exits loop if digitalRead(13) == HIGH
